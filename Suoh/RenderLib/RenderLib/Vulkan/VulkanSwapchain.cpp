...
VulkanSwapchain::VulkanSwapchain(VulkanDevice* device, const VulkanContext& context, u32 width,
                                 u32 height)
...
{
    const vk::Result res = m_Context.device.acquireNextImageKHR(m_Swapchain, ACQUIRE_TIMEOUT,
                                                                m_PresentSemaphores[m_FrameIndex],
                                                                vk::Fence(), &m_ImageIndex);
...
void VulkanSwapchain::InitSwapchain(VulkanDevice* pDevice, const VulkanContext& context, u32 width,
                                    u32 height)
{
...
vk::SurfaceFormatKHR VulkanSwapchain::ChooseSwapchainSurfaceFormat(
    const std::vector<vk::SurfaceFormatKHR>& availableFormats)
{
    for (const auto format : availableFormats)
    {
        // XXX: Have a parameter passed in to choose desired formats instead of hardcoding them
        // here.
    ...
    const std::vector<vk::PresentModeKHR>& availablePresentModes)
{
    ...
    LOG_ERROR("VulkanSwapchain: Failed to get requested swapchain present mode!");
    return vk::PresentModeKHR::eFifo;
}

vk::Extent2D VulkanSwapchain::ChooseSwapchainExtent(const vk::SurfaceCapabilitiesKHR& capabilities,
...
void VulkanSwapchain::InitSemaphores()
{
    ...
    {
        VK_CHECK_ABORT(
            m_Context.device.createSemaphore(&semaphoreInfo, nullptr, &m_PresentSemaphores[i]));
    }
}
