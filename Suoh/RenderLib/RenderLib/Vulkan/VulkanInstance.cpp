...
VulkanDeviceContext VulkanInstance::CreateDevice(const DeviceDesc& desc)
{
    ...
    // device.getQueue(indices.transferFamily, 0, &result.transferQueue);
    ...
    result.transferFamily = indices.transferFamily;
                                                                     ...
                                                                     const DeviceDesc& desc) const
{
    ...
    indices.graphicsFamily = u32(-1);
    indices.computeFamily = u32(-1);
    indices.presentFamily = u32(-1);
    indices.presentFamily = u32(-1);
    ...
    for (int i = 0; i < properties.size(); i++)
    {
        ...
        if (indices.graphicsFamily == u32(-1))
        {
        ...
        if (indices.computeFamily == u32(-1))
        {
        ...
        if (indices.presentFamily == u32(-1))
        {
        ...
        if (indices.transferFamily == u32(-1))
        {
            if (props.queueFlags & vk::QueueFlagBits::eTransfer)
            {
                indices.transferFamily = i;
                LOG_INFO("Transfer family set!\n");
            }
        }

        if (indices.graphicsFamily != u32(-1) && indices.computeFamily != u32(-1)
            && indices.presentFamily != u32(-1) && indices.transferFamily != u32(-1))
            break;
    }
