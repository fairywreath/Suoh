...
VkFormat toVkFormat(ImageFormat format)
{
    ...
    default:
        assert("Unknown ImageFormat!"); // We have tried to convert a image format we don't know about, did we just add it?
    }
...
VkFormat toVkFormat(DepthImageFormat format)
{
    switch (format)
    {
    case DepthImageFormat::UNKNOWN:                    assert(false); break;
...
VkSampleCountFlagBits toVkSampleCount(SampleCount count)
{
    ...
    case SampleCount::SAMPLE_COUNT_16: return VK_SAMPLE_COUNT_16_BIT;
    default:
