...
namespace detail
{
template <class Tag, typename T> T underlying_type(strong_typedef<Tag, T>);
...
template <class Tag, typename T> TYPE_SAFE_CONSTEXPR14 T& get(strong_typedef<Tag, T>& type) noexcept
{
    return static_cast<T&>(type);
}

/// \group strong_typedef_get
template <class Tag, typename T> constexpr const T& get(const strong_typedef<Tag, T>& type) noexcept
{
...
template <typename From, typename To>
using enable_if_convertible_same =
    typename std::enable_if<std::is_convertible<typename std::decay<From>::type, To>::value>::type;
...
template <class T>
constexpr typename std::enable_if<!is_strong_typedef<T>::value, T&&>::type forward_or_underlying(
    T&& type) noexcept
{
...
template <class StrongTypedef> struct equality_comparison
{
};
TYPE_SAFE_DETAIL_MAKE_OP(==, equality_comparison, bool)
TYPE_SAFE_DETAIL_MAKE_OP(!=, equality_comparison, bool)

template <class StrongTypedef, typename Other> struct mixed_equality_comparison
{
};
TYPE_SAFE_DETAIL_MAKE_OP_MIXED(==, mixed_equality_comparison, bool)
TYPE_SAFE_DETAIL_MAKE_OP_MIXED(!=, mixed_equality_comparison, bool)

template <class StrongTypedef> struct relational_comparison
{
};
...
template <class StrongTypedef, typename Other> struct mixed_relational_comparison
{
};
...
template <class StrongTypedef> struct explicit_bool
{
    /// \exclude
...
template <class StrongTypedef> struct increment
{
    /// \exclude
...
template <class StrongTypedef> struct decrement
{
    /// \exclude
    TYPE_SAFE_CONSTEXPR14 StrongTypedef& operator--()
    {
...
template <class StrongTypedef> struct unary_plus
{
};
...
/// \exclude
template <class StrongTypedef> constexpr StrongTypedef operator+(unary_plus<StrongTypedef>&& lhs)
{
    return StrongTypedef(+get(static_cast<StrongTypedef&&>(lhs)));
}

template <class StrongTypedef> struct unary_minus
{
};
...
/// \exclude
template <class StrongTypedef> constexpr StrongTypedef operator-(unary_minus<StrongTypedef>&& lhs)
{
    return StrongTypedef(-get(static_cast<StrongTypedef&&>(lhs)));
}
...
template <class StrongTypedef> struct complement
{
};
...
/// \exclude
template <class StrongTypedef> constexpr StrongTypedef operator~(complement<StrongTypedef>&& lhs)
{
    return StrongTypedef(~get(static_cast<StrongTypedef&&>(lhs)));
}
...
template <class StrongTypedef, typename IntT> struct bitshift
{
};
...
template <class StrongTypedef, typename Result, typename Index = std::size_t> struct array_subscript
{
    /// \exclude
...
struct TYPE_SAFE_MSC_EMPTY_BASES input_iterator
...
template <class StrongTypedef, typename T, typename Distance = std::ptrdiff_t>
struct TYPE_SAFE_MSC_EMPTY_BASES output_iterator
...
template <class StrongTypedef, typename T, typename Distance = std::ptrdiff_t>
struct TYPE_SAFE_MSC_EMPTY_BASES bidirectional_iterator
...
template <class StrongTypedef, typename T, typename Distance = std::ptrdiff_t>
struct TYPE_SAFE_MSC_EMPTY_BASES random_access_iterator
...
template <class StrongTypedef> struct input_operator
{
    /// \exclude
...
template <class StrongTypedef> struct output_operator
{
    /// \exclude
    template <typename Char, class CharTraits>
